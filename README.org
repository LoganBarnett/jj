...
Architecture's notion of ports and adapters.  That pattern would go further
and abstract all I/O behind traits, allowing transports to be swapped out.  We
deliberately stop short of that: the external interactions this library
performs /are/ the domain, not infrastructure to be abstracted away.  There is
nothing meaningful to inject.

The practical test: if another team wanted to embed this functionality in
their own tool, they should be able to import the library crate directly
without touching the binary crate.
